...
export default function EditStepBasic({ setStep, product }) {
  ...
  useEffect(() => {
    ...
    setUsageInstructions(
      product.usage_instructions
        ? Object.entries(product.usage_instructions).map(([k, v]) => ({
            key: k,
            value: v,
          }))
        : [{ key: "", value: "" }],
  ...
  const handleUsageChange = (i, field, value) => {
  ...
  const addUsage = () =>
    setUsageInstructions([...usageInstructions, { key: "", value: "" }]);

  const removeUsage = (i) =>
    setUsageInstructions(usageInstructions.filter((_, idx) => idx !== i));
  ...
  const handleSubmit = async () => {
    ...
    const formattedUsage = usageInstructions
      .filter((s) => s.key && s.value)
      .reduce((acc, cur) => {
        acc[cur.key] = cur.value;
        return acc;
    ...
    try {
      ...
      await api.post(`/admin-dashboard/update-product/${product.id}`, {
        ...
        usage_instructions: formattedUsage,
      ...
      <div className="relative overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
        ...
        <div className="relative flex flex-col gap-5 p-6 md:flex-row md:items-start md:justify-between md:p-8">
          <div className="space-y-2">
            ...
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-slate-500">
                Update the core product information, category mapping, and
                content sections before moving to the next edit steps.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 md:min-w-[360px]">
            ...
            <StatCard
              label="Sections"
              value={String(tabs.length).padStart(2, "0")}
            />
          </div>
        </div>
      </div>
              ...
              <div className="rounded-3xl border border-slate-200 bg-slate-50/70 p-5">
                <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">
                      Add step-by-step usage instructions for this product.
                    </p>
                  </div>
                  <button
                    onClick={addUsage}
                  ...
                  {usageInstructions.map((item, i) => (
                    ...
                    >
                      ...
                      <input
                        ...
                        value={item.key}
                        onChange={(e) =>
                          handleUsageChange(i, "key", e.target.value)
                        }
                      />
                      <input
                        placeholder="Value"
                        ...
                        value={item.value}
                        onChange={(e) =>
                          handleUsageChange(i, "value", e.target.value)
                        }
                      />
                      {usageInstructions.length > 1 && (
                        <button
