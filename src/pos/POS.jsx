...
export default function POS() {
  ...
  const addVariantToCart = (product, variant) => {
    ...
    setCart((prev) => {
      const index = prev.findIndex(
        (i) => i.product_id === product.id && i.variation_id === variant.id,
      );
      ...
      return [
        ...prev,
        {
          ...
          hsn_code: variant.hsn_code, // <-- ADD HERE
          price: variant.price,
          MRP: variant.MRP,
  ...
  const handleSearch = async (value) => {
    ...
    try {
      const res = await api.get(
        "/admin-dashboard/offline-store/pos-products-search",
        {
          params: { search: value },
        },
      );
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const refresh = await api.get(
        "/admin-dashboard/offline-store/pos-products",
        {
          params: { category },
        },
      );
  ...
  const handleAddVariantss = (variant) => {
    ...
    console.log("HSN", variant.hsn_code);
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Fast billing, barcode-ready checkout, and touch-friendly product
                selection.
              </p>
            </div>
